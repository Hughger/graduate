...
// Author: XXX
// Date: 2025-09-16
// Description: AXI to BRAM Bridge (64KB) for ASIC
// Address width: 16 bits 
// Data width: 64 bits
//--------------------------------------------------------------------------------

module axi2bram_asic (
    // AXI Clock and Reset
    ...
    // BRAM Interface
    output wire [15:0] BRAM_PORTA_0_addr,   // NOTE: 16bit
    output wire        BRAM_PORTA_0_clk,
    inout  wire [63:0] BRAM_PORTA_0_dinout, // NOTE: inout port for DATA
    output wire        BRAM_PORTA_0_en,     // enable signal
    output wire        BRAM_PORTA_0_rstn,
    output wire        BRAM_PORTA_0_we      // 1:write, 0:read
);
    ...
    localparam BRAM_ADDR_WIDTH = 16; // BRAM address width (word aligned, 64KB address space)
    ...
    reg [19:0] ax_req_addr_q, ax_req_addr_d;
    reg [7:0] ax_req_len_q, ax_req_len_d;
    reg [2:0] ax_req_size_q, ax_req_size_d;
    reg [1:0] ax_req_burst_q, ax_req_burst_d;
    reg [19:0] req_addr_q, req_addr_d;
    reg [7:0] cnt_q, cnt_d;

    // Internal signals
    wire [19:0] aligned_address;
    wire [19:0] wrap_boundary;
    wire [19:0] upper_wrap_boundary;
    wire [19:0] cons_addr;
    reg req_o, we_o;
    reg [19:0] addr_o;
    ...
    assign BRAM_PORTA_0_rstn = s_axi_aresetn_0;

    // Address generation logic
    assign aligned_address = {ax_req_addr_q[19:3], 3'b000};
    ...
    function [19:0] get_wrap_boundary;
        input [19:0] unaligned_address;
        input [7:0] len;
        begin
            get_wrap_boundary = 20'h0;
            if (len == 8'h01)
                get_wrap_boundary = {unaligned_address[19:4], 4'h0};
            else if (len == 8'h03)
                get_wrap_boundary = {unaligned_address[19:5], 5'h0};
            else if (len == 8'h07)
                get_wrap_boundary = {unaligned_address[19:6], 6'h0};
            else if (len == 8'h0F)
                get_wrap_boundary = {unaligned_address[19:7], 7'h0};
        end
    endfunction

    // BRAM Interface outputs
    // Convert 20-bit AXI address to 16-bit BRAM word address (word aligned)
    // For 64-bit data width, byte address to word address conversion: addr_word = addr_byte >> 3
    // So BRAM word address = addr_o[18:3] (take bits 18 down to 3 from byte address)
    assign BRAM_PORTA_0_addr = addr_o[18:3];  // Correct word address mapping
    //assign BRAM_PORTA_0_din = S_AXI_0_wdata;
    assign BRAM_PORTA_0_en = req_o;
    assign BRAM_PORTA_0_we = we_o;

    // BRAM data port control for inout interface
    assign BRAM_PORTA_0_dinout = (req_o && we_o) ? S_AXI_0_wdata : 64'hz;
    ...
    // For read operations: read data from BRAM
    // For write operations: rdata is don't care (AXI spec doesn't use it)
    assign S_AXI_0_rdata = BRAM_PORTA_0_dinout;

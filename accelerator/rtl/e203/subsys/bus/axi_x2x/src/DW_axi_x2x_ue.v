...
module DW_axi_x2x_ue (
 ...
 //outputs
  ...
  //parameters
  parameter MAX_ASIZE_S = (`X2X_MAX_SP_ASIZE > 0) ? `X2X_MAX_SP_ASIZE : 1; //ByteSize-->BurstSize
  ...
  // is by default 32 bit, spyglass is considering it as a vector. Functionally
  // there will not be any issue as negation of it will be either 0 or 1
  assign xact_upsize_o = ( `X2X_UPSIZE_ANY_ASIZE | 
                           (!`X2X_UPSIZE_ANY_ASIZE & (`X2X_LOG2_MP_SW == asize_i)) ) &
                           `X2X_HAS_TX_UPSIZE& ( `X2X_MP_DW < `X2X_SP_DW ) &

...
//  parameter X2X_TX_UPSIZE     = `X2X_HAS_TX_UPSIZE; //upsize = 1
  ...
  parameter ET_BYPASS     = NO_ALTER && ((`X2X_HAS_ET==0)); //1->bypass ET,
  ...
  parameter [0:0] PL                = `X2X_HAS_PIPELINE; //pipeline
  ...
  assign aburst_in = (aburst_i == `X2X_BT_WRAP) ?
                     `X2X_BT_INCR : aburst_i;
 ...
 //For PL
  ...
  //asize is SP data width.
    assign reduce_num = {RDC_NUM_W{1'b0}};
...
parameter [2:0] MAX_ASIZE_S_WIRE_1 = `X2X_MAX_SP_ASIZE; //MAX_ASIZE_S;
parameter [LOG2_BIG_BW:0] MAX_SP_TOTAL_BYTE_WIRE_1 = `X2X_MAX_SP_TOTAL_BYTE; //max_sp_asize *
  ...
  parameter [2:0] MAX_SP_ASIZE_WIRE             = `X2X_MAX_SP_ASIZE; //bytes -> asize
  parameter [MP_BLW:0] MAX_SP_ALEN_WIRE_2       = `X2X_MAX_SP_ALEN+1;//{`X2X_SP_BLW{1b1}}+1

  assign spasize_larger = ((asize_temp) <= MAX_SP_ASIZE_WIRE) |
                          SPSIZE_LRG_MP;
    assign spalen_larger = (mp_alen <= MAX_SP_ALEN_WIRE_2) | SPALEN_LRG_MP;
    assign spbyte_larger = (mp_total_byte <= `X2X_MAX_SP_TOTAL_BYTE) | SPBYTE_LRG_MP;
       ...
       if ( last_rs_xact_ctl_r ) begin //in last xact
         if ( spasize_larger ) begin
           ...
           if ( ( spbyte_larger && (
               ...
               ( ((pl_total_remain_bytes <= `X2X_MAX_SP_TOTAL_BYTE) |
                  SPBYTE_LRG_MP) &&

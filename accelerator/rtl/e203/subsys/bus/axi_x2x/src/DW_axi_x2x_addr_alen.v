...
// Revision: $Id: //dwh/DW_ocb/DW_axi_x2x/amba_dev/src/DW_axi_x2x_addr_alen.v#12 $ 
//
/////////////////////////////////////////////////////////////////////////
...
//  parameter MAX_SP_ALEN       = `X2X_MAX_SP_ALEN+1;//{`X2X_SP_BLW{1b1}}+1
  ...
  assign align_plus_sp_total = aligned_addr_i + `X2X_MAX_SP_TOTAL_BYTE;
  assign alen_s  = (mp_total_byte_i >> `X2X_MAX_SP_ASIZE) - 1;
  ...
  //SJ : Here although by expression (total_alen_r_i << asize_i) precision will be lost it doesn't affect the functionality as our intention is to get total number of bytes that can be transfered in a burst.
  assign alen_s56 = (resize_ctrl_i == `MUL_XACT_OR_US_NOTLAST &&
                     (!xact_upsize_i)) ?
                     ((mp_total_byte_i-
                      (total_alen_r_i << asize_i)) >> asize_i) - 1 :
                     ((mp_total_byte_i -
                      (total_alen_r_i << `X2X_MAX_SP_ASIZE))
                      >> `X2X_MAX_SP_ASIZE ) - 1;
  // spyglass enable_block SelfDeterminedExpr-ML
  // spyglass enable_block TA_09

  assign asize_mux = (resize_ctrl_i == `MUL_XACT_OR_US_NOTLAST) ?
                      asize_i : `X2X_MAX_SP_ASIZE;
  ...
  parameter [SP_BLW-1:0]   MAX_SP_ALEN_DFLT       = {`X2X_SP_BLW{1'b1}};
  parameter [LOG2_MP_BW:0] MAX_SP_TOTAL_ALEN_DFLT       = `X2X_MAX_SP_ALEN+1;//{`X2X_SP_BLW{1b1}}+1
  always @( micro_ctrl_i or alen_s or
            align_plus_mpsp_total or 
            alen_i or
            total_alen_r_i or total_addr_r_i or align_plus_sp_total
          ) begin: MXUI_CTRL_PROC
    mxui_alen = alen_i;
    ...
    mxui_total_addr = total_addr_r_i;
      ...
      case ( micro_ctrl_i )
        ...
        `MXUI_NO_CROSS, `MXUI_FI_MUL, `MXUI_FI_DS: begin
          mxui_total_alen = MAX_SP_TOTAL_ALEN_DFLT;
          mxui_alen = MAX_SP_ALEN_DFLT; // MAX_SP_ALEN - 1;
...
//SJ : Here mxui_total_addr is assigned multiple times depend upon various conditions and expression as per the requirement.
  ...
  always @( micro_ctrl_i or 
            ...
            aburst_i or plus_sp_mux ) begin: MXUN_CTRL_PROC
    mxun_alen = alen_i;
    ...
    mxun_total_addr = total_addr_r_i;
      ...
      case ( micro_ctrl_i )
        ...
        `MUX_FI_MUL: begin
          mxun_alen = MAX_SP_ALEN_DFLT; // MAX_SP_ALEN - 1;
...
//spyglass enable_block W415a
  ...
  always @( micro_ctrl_i or 
            ...
            align_plus_sp_total or reduce_num_r_i ) begin:MXI_CTRL_PROC
    mxi_alen = alen_i;
    mxi_stall = 1'b0; 
      ...
      case ( micro_ctrl_i )
        ...
        `MXI_FI: begin
          if ( aburst_i == `X2X_BT_FIXED )
            mxi_alen = MAX_SP_ALEN_DFLT; // MAX_SP_ALEN - 1;
          else begin //INCR
            if ( (`X2X_MAX_SP_ALEN+1) > reduce_num_i ) begin
  // spyglass disable_block TA_09
  // SMD: Reports cause of uncontrollability or unobservability and estimates the number of nets whose controllability / observability is impacted.
  // SJ : Since some bits of the both operands are tied to VCC or ground for all configurations, we are getting uncontrollability issue here. But as per the design logic it is valid and we can't force controllability on this net. Hence waiving it.
              mxi_alen = MAX_SP_ALEN_DFLT - reduce_num_i; // MAX_SP_ALEN - 1 - reduce_num_i;
  // spyglass enable_block TA_09
              mxi_reduce_num = {RDC_NUM_W{1'b0}};
            end
            else begin //MAX_SP_ALEN <= reduce_num_i
              mxi_alen = MAX_SP_ALEN_DFLT; // MAX_SP_ALEN - 1;
              mxi_stall = 1'b1;
              mxi_reduce_num = reduce_num_i - (`X2X_MAX_SP_ALEN+1);//MAX_SP_ALEN;
            end
          end

          mxi_total_alen = MAX_SP_TOTAL_ALEN_DFLT;
  ...
  assign total_plus_sp_total = total_addr_r_i + `X2X_MAX_SP_TOTAL_BYTE;//MAX_SP_TOTAL_BYTE;
//spyglass disable_block W415a 
//SMD: Signal may be multiply assigned (beside initialization) in the same scope.
//SJ : Here mxun_addr is assigned multiple times depend upon various conditions and expression as per the requirement.
  always @( micro_ctrl_i or 
  ...
  //spyglass disable_block SelfDeterminedExpr-ML
  //SMD: Self determined expression present in the design.
  //SJ : Here although expression (total_remain_bytes_i - pre_ran_num_bytes_i) is evaluated first with width LOG2_MP_BW. Then whole assignment statement is evaluated with resepct to mxn_alen width. This is intentional, precision loss is negligible. 
  ...
  // LMD: Unequal length in DEFAULT, LHS : 4, RHS : 1 
  // LJ: This is not an issue functionally as the missing MSBs will be appended with zeros.
  ...
  // spyglass enable_block W116
        ...
        `MUX_FI_MUL: begin
          if ( aburst_i == `X2X_BT_FIXED )
            mxn_alen = MAX_SP_ALEN_DFLT; // MAX_SP_ALEN - 1;
          else begin //`X2X_BT_INCR
            if ( (`X2X_MAX_SP_ALEN+1) > reduce_num_r_i ) begin
  // spyglass disable_block TA_09
  // SMD: Reports cause of uncontrollability or unobservability and estimates the number of nets whose controllability / observability is impacted.
  // SJ : Since some bits of the both operands are tied to VCC or ground for all configurations, we are getting uncontrollability issue here. But as per the design logic it is valid and we can't force controllability on this net. Hence waiving it.
              mxn_alen = MAX_SP_ALEN_DFLT - reduce_num_r_i; // MAX_SP_ALEN - 1 - reduce_num_r_i;
  // spyglass enable_block TA_09
            end
            else begin //MAX_SP_ALEN <= reduce_num_i
              mxn_alen = MAX_SP_ALEN_DFLT; // MAX_SP_ALEN - 1;
          ...
          if ( aburst_i == `X2X_BT_INCR ) begin
            ...
            if ( aready_i ) begin
              ...
              if ( (`X2X_MAX_SP_ALEN+1) > reduce_num_r_i )
                mxn_reduce_num = {RDC_NUM_W{1'b0}};
              else
                mxn_reduce_num = reduce_num_r_i - (`X2X_MAX_SP_ALEN+1);
            end
          end
        end
 ...
 //spyglass enable_block W415a
  ...
  always @( reduce_num_r_i or mxi_reduce_num or micro_ctrl_i or 
            mxn_reduce_num ) begin: MXIN_RED_PROC

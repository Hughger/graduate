...
//spyglass disable_block W528
//SMD: A signal or variable is set but never read.
//SJ : bus_pyld_rpck_chf_mux is read only when X2X_WI_FAN_OUT is defined and TX_ALTER != `X2X_TX_NO_ALTER.
// spyglass disable_block SelfDeterminedExpr-ML
// SMD: Self determined expression found
// SJ: The expression indexing the vector/array will never exceed the bound of the vector/array.
...
//spyglass enable_block W415a
  ...
  // spyglass enable_block W576
                     ...
                     // payload mux.
  assign payload_o = (TX_ALTER != `X2X_TX_NO_ALTER) ? bus_pyld_rpck_chf_mux[SP_PYLD_W-1:0]
...
/*  assign bus_ready_o = ((TX_ALTER != `X2X_TX_NO_ALTER) || WI_FAN_OUT)
  ...
  */ 
  ...
  else if ((TX_ALTER != `X2X_TX_NO_ALTER) && (NUM_RSI_FIFOS > 1))
    assign bus_ready_o = |(~bus_rsif_pop_n);
  else if ((TX_ALTER != `X2X_TX_NO_ALTER))
    assign bus_ready_o = (~bus_rsif_pop_n);
  else

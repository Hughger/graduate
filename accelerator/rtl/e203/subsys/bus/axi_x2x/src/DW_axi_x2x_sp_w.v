...
//----------------------------------------------------------------------
  ...
  parameter MP_ID_W             = `X2X_MP_IDW;          // Internal ID width will always be this value. May be padded
  ...
  parameter RSI_FIFO_W          = `X2X_SP_W_RSFIFO_W;   // Width of resize info. fifo.
  parameter RSINFO_FIFO_D       = `X2X_SP_W_RSFIFO_D;   // Depth of resizing info. fifo.
  parameter RSINFO_FIFO_D_L2    = `X2X_SP_W_RSFIFO_D_L2;// Log base 2 of RSINFO_FIFO_D.
  parameter ADDR_TRK_W          = `X2X_ADDR_TRK_W;      // Bits of address that X2X requires for resizing.
  parameter BEATS_PER_POP_W     = `X2X_SP_W_BEATS_PER_POP_W; // Beats per pop width.     
  parameter WID                 = (`X2X_HAS_WI_FAN_OUT ? 1 : `X2X_WID); // Write interleaving depth.
  ...
  parameter MP_DW               = `X2X_MP_DW;           // MP signal width parameters.
  parameter MP_SW               = `X2X_MP_SW;           // MP signal width parameters.
  parameter SP_DW               = `X2X_SP_DW;           // SP signal width parameters.
  parameter SP_SW               = `X2X_SP_SW;           // SP signal width parameters.
  parameter DATA_SEL_W          = `X2X_DATA_SEL_W;      // Data select width.
  parameter MP_LOG2_SW          = `X2X_LOG2_MP_SW;      // Log base 2 of MP strobe width.
 parameter LOG2_MAX_SW          = `X2X_LOG2_MAX_SW;     // Log b2 of max strobe width.
...
//----------------------------------------------------------------------
  ...
  // This signal is used only when (`X2X_TX_ALTER==`X2X_TX_NO_ALTER)
  wire               mp_wlast;
...
// Depedning on the MP/SP data bus widths, some LSBs of this signal maybe unused. This is not an issue.
  ...
  wire [BUS_BEATS_PER_POP_W-1:0] update_bpp_nxt; 
...
//spyglass enable_block W415a
  ...
  //spyglass disable_block W528
  //SMD: A signal or variable is set but never read.
  //SJ : mp_wlast will be read only when `X2X_TX_ALTER==`X2X_TX_NO_ALTER.
  assign {
    mp_id, 
    mp_wdata, 
    mp_wstrb, 
...
// spyglass enable_block SelfDeterminedExpr-ML
  ...
  begin : update_addr_nxt_PROC
    ...
    case({beat_acc, mux_aburst})
      ...
      {1'b1, `X2X_BT_FIXED} : begin
        ...
        //VCS coverage off
        if(mux_asize_mp >  `X2X_MAX_SP_ASIZE) begin
  ...
  //--------------------------------------------------------------------
  assign update_bpp_nxt = {BUS_BEATS_PER_POP_W{1'b0}};
  ...
  begin : pop_req_n_o_PROC
    ...
    if(~pop_empty_i) begin
      ...
      // multiple MP beats for a single SP beat.
        ...
        pop_req_n_o = 
          ((`X2X_HAS_TX_UPSIZE & upsized_tx) == 1'b1)
  ...
  // from or vice versa. ADDR_TRK_W and DATA_SEL_W are generated in
  // the tcl plugin file.
  assign sp_data_sel 
    = current_addr[(ADDR_TRK_W-1):MP_LOG2_SW];


  // When SP DW is larger than MP DW we require demultiplexers
  // to generate the correct data and strobe signals.
  
  // Data demux.
  DW_axi_x2x_busdemux
  
  #(`X2X_RS_RATIO, // Number of buses to mux between.
    MP_DW,         // Width of bus input to the mux.
    DATA_SEL_W     // Width of select line.
  ) 
  U_sp_dw_larger_data_demux (
    .sel   (sp_data_sel),
    .din   (et_mp_wdata),
    .dout  (sp_nus_wdata)
  );
  ...
  #(`X2X_RS_RATIO, // Number of buses to mux between.
    MP_SW,         // Width of each bus input to the mux.
    DATA_SEL_W     // Width of select line.
  ) 
  U_sp_dw_larger_strobe_demux (
    .sel   (sp_data_sel),
    .din   (et_mp_wstrb),
    .dout  (sp_nus_wstrb)
  );

  // Muxes to select upsized data and strobes or mapped data and
  // strobes. Control with valid out so we won't get data or 
  // strobes changing when valid is not asserted (Not a protocol
  // problem but waves look better if we stick to this.)
  assign sp_wdata = sp_nus_wdata;
  assign sp_wstrb = sp_nus_wstrb;
  ...
  // Decode valid out.
  generate if(   (`X2X_TX_ALTER == `X2X_TX_NO_ALTER) && (`X2X_HAS_ET == 0))
  always @(*)
  begin : valid_o1_PROC
    ...
    // spyglass disable_block W528
    // SMD: A signal or variable is set but never read.
    // SJ : us_valid is read only when `X2X_TX_ALTER != `X2X_TX_NO_ALTER or `X2X_HAS_ET == 1.
    valid_o = 1'b0;
    us_valid = 1'b0;
    // spyglass enable_block W528
  ...
  //VCS coverage off
  generate if (`X2X_TX_ALTER==`X2X_TX_NO_ALTER)

...
input    [`REG_PEAK_RATE_W-1:0]          reg_peak_rate_i                 ;
...
//------------------------------------------------------------------------------
// Based on the slave visibility, the particular request is unconnected in the top level DW_axi file.
wire   [`REG_PEAK_RATE_W:0]      peak_rate_cnt     ; // peak rate counter
wire                             peak_rate_flg_set ; // peak rate flag set
reg    [`REG_PEAK_RATE_W:0]      peak_rate_cnt_r   ; // registered peak_rate_cnt
...
assign peak_rate_cnt = reg_regulation_enable_i ? 
                       (peak_rate_cnt_r[`REG_PEAK_RATE_W-1:0] + 
                       reg_peak_rate_i) :{`REG_PEAK_RATE_W{1'b0}};
// spyglass enable_block W164b

//------------------------------------------------------------------------------
// peak_rate_flg_set is asserted when peak_rate_cnt MSB is set to 1.
//------------------------------------------------------------------------------
assign peak_rate_flg_set = peak_rate_cnt[`REG_PEAK_RATE_W];
...
begin: peak_rate_cnt_r_PROC
   if (~aresetn_i)   peak_rate_cnt_r <= {(`REG_PEAK_RATE_W+1){1'b0}};
...
begin:peak_rate_flg_PROC 
   if (~aresetn_i)   peak_rate_flg <= 1'b0;
           ...
           (reg_peak_rate_i == {`REG_PEAK_RATE_W{1'b0}}))) peak_rate_flg <= 1'b1;

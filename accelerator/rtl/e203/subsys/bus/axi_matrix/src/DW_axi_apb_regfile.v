...
`define REG_HWC             4'h4
`define REG_CMD             4'h8
`define REG_DATA            4'hC
`define COMMAND_EN_BIT      31
`define WR_BIT              30
`define SR_BIT              29
`define MID_BIT_MSB         11
`define CH_ID_BIT           7
`define QOS_REG_OFFSET      3 

module DW_axi_apb_regfile (
     pclk,
     presetn,
     wr_en,
     rd_en,
   ...
   //------------------------------------------------------
     ...
     input  [`IC_ADDR_SLICE_LHS-2:0]    reg_addr;       //APB register offset
     input  [`MAX_APB_DATA_WIDTH-1:0]   ipwdata;        //Write data from APB BIU 
     output [`MAX_APB_DATA_WIDTH-1:0]   iprdata;        //Internal read data bus
  ...
  //Local Parameters
  localparam [31:0] VID_REG         = `DW_AXI_VERSION_ID; //  version id of corekit , driven by coreConsultant param
  localparam        APB3BIT         = `AXI_HAS_APB3;
  localparam        QOSBIT          = `AXI_HAS_QOS;
  localparam        LOCKENBIT       = `AXI_HAS_LOCKING;
  localparam        TZENBIT         = `AXI_HAS_TZ_SUPPORT;
  localparam        EXTDECBIT       = `AXI_HAS_XDCDR;
  localparam        REMAPENBIT      = `AXI_REMAP_EN;
  localparam        BICMDENBIT      = `AXI_HAS_BICMD;
  localparam  [0:0] AXI_HAS_AXI4    = (`AXI_INTERFACE_TYPE == 0) ? 0 : 1;
  localparam        LPENBIT         = `AXI_LOWPWR_HS_IF; 
  localparam  [4:0] MST_NUM         = `AXI_NUM_MASTERS;
  localparam  [4:0] SLV_NUM         = `AXI_NUM_SLAVES;
  ...
  // ------------------------------------------------------
   ...
   //  SW-registers 
   // ------------------------------------------------------
   assign apb_reg_vid_en       = (reg_addr == (`REG_VID >> 2));
   assign apb_reg_hwconfig_en  = (reg_addr == (`REG_HWC >> 2));
   assign apb_reg_cmd_en       = (reg_addr == (`REG_CMD >> 2));
   assign apb_reg_data_en      = (reg_addr == (`REG_DATA >> 2));
   ...
   always @(posedge pclk or negedge presetn) begin : APBWR_CMDREG_WRITE_PROC
        if(presetn == 1'b0) 
            ...
            {cmd_reg[31],cmd_reg[29]}              <= {(cmd_reg[31] & ((~command_en_ack) | (cmd_reg[`SR_BIT] & (~internal_rst)))), 
                                                       (cmd_reg[31] & (~command_en_ack) & cmd_reg[`SR_BIT])}; //Auto clearing of cmd_reg bits  
  ...
  always @(posedge pclk or negedge presetn) begin : APBWR_DATAREG_WRITE_PROC
    ...
    always @(*)       begin : APB_REG_READ_PROC
     ...
     assign command_en       =   cmd_reg[`COMMAND_EN_BIT]; 
     ...
     assign internal_rst =   command_en_ack & cmd_reg[`SR_BIT];
     assign internal_reg_rst =   cmd_reg[`SR_BIT];
     ...
     assign qosreg_wr_en      =   cmd_reg[`WR_BIT]  &  command_en ; 
            // write command for internal qos register
     
     assign qosreg_rd_en      =   (~cmd_reg[`WR_BIT])  & command_en ;
            // read command for internal qos register
     
     assign qosreg_offset   =   {cmd_reg[`MID_BIT_MSB],cmd_reg[`MID_BIT_MSB-1],cmd_reg[`MID_BIT_MSB-2] ,cmd_reg[`MID_BIT_MSB-3], cmd_reg[`CH_ID_BIT] ,cmd_reg[`QOS_REG_OFFSET-1:0] }; 

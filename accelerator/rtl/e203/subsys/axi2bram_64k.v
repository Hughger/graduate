...
// Author: Assistant
// Date: 2025-01-XX
// Description: AXI to 256-bit BRAM Bridge for 64KB
...
module axi2bram_64k (
    ...
    input  wire [31:0] S_AXI_araddr,    // 32-bit for slave address space
...
reg [31:0] ax_req_addr_q, ax_req_addr_d;  // AXI byte address (32-bit)
reg [7:0] ax_req_len_q, ax_req_len_d;
reg [2:0] ax_req_size_q, ax_req_size_d;
reg [1:0] ax_req_burst_q, ax_req_burst_d;
reg [10:0] req_addr_q, req_addr_d;        // SRAM word address (11-bit)
reg [7:0] cnt_q, cnt_d;
...
// BRAM interface signals - RAM1 (0x0_0000~0x0_FFFF)
reg [10:0] bram1_w_addr_o, bram1_r_addr_o;
reg [255:0] bram1_w_din_o;
reg         bram1_w_en_o, bram1_r_en_o;
reg [31:0]  bram1_w_we_o;

// BRAM interface signals - RAM2 (0x1_0000~0x1_FFFF)
reg [10:0] bram2_w_addr_o, bram2_r_addr_o;
...
// Function to convert 32-bit AXI address to relative address (20-bit for 1MB space)
function [19:0] axi_addr_to_relative;
    input [31:0] axi_addr;
    begin
        // Extract bits [19:0] for 1MB address space within each slave
        axi_addr_to_relative = axi_addr[19:0];
    end
endfunction

// Function to determine which RAM to access (0=RAM1, 1=RAM2)
function is_ram2;
    input [31:0] axi_byte_addr;
    reg [19:0] relative_addr;
    begin
        relative_addr = axi_addr_to_relative(axi_byte_addr);
        // RAM1: 0x0_0000 ~ 0x0_FFFF (bit[16] = 0)
        // RAM2: 0x1_0000 ~ 0x1_FFFF (bit[16] = 1)
        is_ram2 = relative_addr[16];
    end
endfunction

// Function to convert AXI byte address to ASIC SRAM word address
function [10:0] axi_addr_to_sram_word;
    input [31:0] axi_byte_addr;
    reg [19:0] relative_addr;
    reg [19:0] ram2_addr;
    begin
        relative_addr = axi_addr_to_relative(axi_byte_addr);
        if (relative_addr[16]) begin
            // RAM2: subtract 0x10000 offset and use bits [15:5] for 64KB address space
            // 0x10000 -> 0x000, 0x1FFFF -> 0x7FF (2047)
            ram2_addr = relative_addr - 20'h10000;
            axi_addr_to_sram_word = ram2_addr[15:5];
        end else begin
            // RAM1: use bits [15:5] for 64KB address space
            axi_addr_to_sram_word = relative_addr[15:5];
        end
    end
endfunction
...
// BRAM1 outputs (First RAM: 0x0_0000~0x0_FFFF)
...
// BRAM2 outputs (Second RAM: 0x1_0000~0x1_FFFF)
...
always @(*) begin
    ...
    bram1_w_addr_o = 11'h0;
    bram1_w_din_o = 256'h0;
    bram1_w_en_o = 1'b0;
    bram1_w_we_o = 32'h0;
    bram1_r_addr_o = 11'h0;
    bram1_r_en_o = 1'b0;

    // BRAM2 signals default
    bram2_w_addr_o = 11'h0;
    bram2_w_din_o = 256'h0;
    bram2_w_en_o = 1'b0;
    bram2_w_we_o = 32'h0;
    bram2_r_addr_o = 11'h0;
    bram2_r_en_o = 1'b0;
    ...
    case (state_q)
        IDLE: begin
            ...
            if (S_AXI_arvalid) begin
                ...
                state_d = READ_SETUP;
                req_addr_d = axi_addr_to_sram_word(align_to_256bit(axi_addr_to_relative(S_AXI_araddr)));
                cnt_d = 8'h0;
                burst_cnt_d = get_burst_position(axi_addr_to_relative(S_AXI_araddr));  // Initialize based on address offset
                ...
                state_d = WRITE_SETUP;
                req_addr_d = axi_addr_to_sram_word(align_to_256bit(axi_addr_to_relative(S_AXI_awaddr)));
                cnt_d = 8'h0;
                burst_cnt_d = get_burst_position(axi_addr_to_relative(S_AXI_awaddr));  // Initialize based on address offset
            end
        end

        READ_SETUP: begin
            // Prepare to read from BRAM based on address
            if (is_ram2(ax_req_addr_q)) begin
            ...
            if (S_AXI_rready) begin
                if (r_last_o) begin
                    ...
                    if (burst_cnt_q == 2'b11) begin
                        ...
                        req_addr_d = req_addr_q + 11'd1;   // Next SRAM word address
                        burst_cnt_d = 2'b00;
                        state_d = READ_SETUP;
                    end else begin
        ...
        WRITE_SETUP: begin
            ...
            if (S_AXI_wvalid) begin
                // Check if this is a control register write (address 0xF_0000 in relative space)
                if (axi_addr_to_relative(ax_req_addr_q) == 20'hF0000) begin
        ...
        WRITE_COMMIT: begin
            ...
            if (S_AXI_wlast) begin
                ...
                req_addr_d = req_addr_q + 11'd1;  // Next BRAM word address

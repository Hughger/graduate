...
int main(void)
{
    ...
    // 24.3 配置源地址（S7地址）
    apb_write(DESC0_SRC_ADDR, S7_BASE_ADDR);
    short_delay();
    ...
    if (dma_result != 0) {
        // DMA传输失败
        if (dma_result == -1) {
            __asm__ volatile ("li x3, 260");  // S7→S1 DMA错误
        } else {
            __asm__ volatile ("li x3, 261");  // S7→S1 DMA超时
        }
        goto end;
    }
    ...
    for(int i = 0; i < TEST_COUNT; i++) {
        ...
        if (read_data != s7_test_patterns[i]) {

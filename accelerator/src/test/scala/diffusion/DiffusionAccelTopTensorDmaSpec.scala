...
class DiffusionAccelTopTensorDmaSpec extends AnyFlatSpec with ChiselScalatestTester {
  ...
  "DiffusionAccelTop" should "serve tensor-read DMA during LoadResidual through the shared MIG transfer" in {
    test(new DiffusionAccelTop) { dut =>
      ...
      dut.io.activationReadCommand.bits.baseAddress.poke(0.U)
      dut.io.activationReadCommand.bits.vectors.poke(1.U)
      dut.io.activationReadCommand.valid.poke(true.B)
      dut.clock.step()
      dut.io.activationReadCommand.valid.poke(false.B)
      dut.clock.step(64)
      dut.io.activationVector.valid.expect(true.B)
      dut.io.activationVector.bits(0).expect((-1330).S)
      for (lane <- 1 until 32) { dut.io.activationVector.bits(lane).expect(0.S) }
      dut.io.activationVector.ready.poke(true.B)
      dut.clock.step()
      dut.io.activationReadDone.expect(true.B)
    }
  }
}
`timescale 1ns / 1ps
//=============================================================================
// chip_test_0601.v — 最小功能验证（单次 run_process(0,0,0,...)）
//
// 通过条件（必须同时满足）：
//   1. dbg_fsm_state == S_PASS (14)
//   2. dbg_mac_done 在 S_WAIT_DONE 期间曾拉高
//   3. led_mac_done 常亮（mac_done_latch）
//
// 失败：dbg_fsm_state == S_ERROR (15)，原因 mac_err 或等待超时
// 不再用短超时“假完成”进入后续状态。
//
// 地址：chip_add = 字节地址[18:3]（64b 字地址）；配置 reg_id 即字地址
//=============================================================================
...
(
  ...
  localparam [31:0] FSM_NORMAL_CFG  = 32'h0007_EE22;
  ...
  // @16MHz ≈10s；仅作看门狗，成功路径只认 mac_done
  localparam [27:0] MAC_WAIT_LIMIT = 28'd160_000_000;

  // FSM 状态（ILA 看 dbg_fsm_state）
  ...
  localparam S_PASS          = 8'd14;  // 仅 mac_done 可达
  localparam S_ERROR         = 8'd15;  // mac_err 或超时
  ...
  (* mark_debug = "true" *) reg [14:0] feature_idx   = 15'd0;  // ILA 名保持 feature_idx
  ...
  always @(posedge clk_16M or negedge ck_rst) begin
    if (!ck_rst) begin
      ...
      case (fsm_state)
        ...
        // 只认 mac_done 为成功；mac_err/超时进 S_ERROR，不清中断、不假装完成
        S_WAIT_DONE: begin

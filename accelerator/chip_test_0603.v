...
//   S_ERROR 停留约 10s（MAC_WAIT_LIMIT=160M 周期 @16MHz）后自动跳 S_RESET，
//   同时请求芯片硬件复位（fsm_rst_req），确保芯片与 FPGA 同步恢复。
//   瞬态故障无需手动复位；持续故障会在 ILA 中反复出现 S_ERROR 脉冲便于定位。
//
// [FIX-8] 调试信号注释完善
//   feature_idx 是 feat_xfer_idx 的 ILA 镜像（每周期更新），已标注。
//   dbg_chip_data 在写周期显示 data_out_to_chip，读周期显示 data_in_from_chip。
//
// v0603.1 (2026-06-03)：修复 code review 发现的 5 项 bug
//
// [FIX-9] mmcm_locked 统一使用同步版本
//   芯片复位块（chip_rst_n_reg）也改用 mmcm_locked_s2（原用未同步的 mmcm_locked），
//   消除 FSM 与芯片复位之间的 2 周期竞态窗口。与 FIX-2 配合，芯片和 FPGA 控制器
//   在同一拍进入复位。
//
// [FIX-10] S_WAIT_DONE 三条出口统一经 S_CLR_INTR 清中断
//   a) 超时路径（mac_wait_cnt>=LIMIT）现在也走 S_CLR_INTR，不再直跳 S_ERROR
//   b) chip_mac_err_s2 检查优先于 chip_mac_done_s2：若两者同时到达，报错不报 PASS
//   三条路径行为统一，ILA 会稳定看到 0C→0D→0E/0F，不再出现 0C→0F 直跳
//
// [FIX-11] S_ERROR 自动恢复时请求芯片硬件复位
//   S_ERROR 超时后拉高 fsm_rst_req，芯片复位块将 chip_rst_n_reg 拉低，
//   确保芯片内部状态机与 FPGA 同步回到初始态。S_RESET 256 周期后释放请求，
//   芯片再经 256 周期复位计数后释放。总恢复延迟约 512 周期（32μs），
//   而非原来的 10 秒（芯片不复位）。
//
// [FIX-12] mmcm_locked_s2 复位分支补全缺失信号
//   补上 fsm_addr、fsm_data_out 的显式复位，消除总线残留值隐患。
//
// [FIX-13] 注释勘误
//   修正 S_ERROR 停留时间：2.6s → 10s（160M/16MHz=10s）。
//=============================================================================

module chip_test_0603
(
  ...
  always @(posedge clk_16M or negedge ck_rst) begin
    if (!ck_rst) begin
      chip_rst_cnt   <= 8'h00;
      chip_rst_n_reg <= 1'b0;
    end else if (!mmcm_locked_s2) begin  // [FIX-1] 使用同步版本，与 FSM 同拍复位
      chip_rst_cnt   <= 8'h00;
      chip_rst_n_reg <= 1'b0;
    end else if (fsm_rst_req) begin  // [FIX-4] FSM 请求硬件复位（S_ERROR 自动恢复）
      chip_rst_cnt   <= 8'h00;
      chip_rst_n_reg <= 1'b0;
  ...
  // [FIX-4] FSM 请求芯片硬件复位（S_ERROR 自动恢复时使用）
  reg        fsm_rst_req     = 1'b0;
      ...
      if (!mmcm_locked_s2) begin
        ...
        case (fsm_state)
          S_RESET: begin
            ...
            // [FIX-4] 若因 S_ERROR 自动恢复进入，延迟 256 周期后释放硬件复位请求
            if (fsm_wait_cnt == 16'h0100)
              fsm_rst_req <= 1'b0;
          ...
          S_WAIT_DONE: begin
            ...
            // [FIX-3] 先检查 err：若 err 与 done 同时到达，err 优先，避免假 PASS
            if (chip_mac_err_s2) begin
              test_active  <= 1'b0;
              mac_wait_cnt <= 28'h0;
              fsm_state    <= S_CLR_INTR;
            end else if (chip_mac_done_s2) begin
              test_active    <= 1'b0;
              mac_done_latch <= 1'b1;
              mac_wait_cnt   <= 28'h0;
              fsm_state      <= S_CLR_INTR;
            end else if (mac_wait_cnt >= MAC_WAIT_LIMIT) begin
              // [FIX-2] 超时也经 S_CLR_INTR 清中断，不再直跳 S_ERROR
              test_active  <= 1'b0;
              mac_wait_cnt <= 28'h0;
              fsm_state    <= S_CLR_INTR;
            end
          ...
          S_ERROR: begin
            ...
            if (error_wait_cnt >= MAC_WAIT_LIMIT) begin
              error_wait_cnt <= 28'h0;
              fsm_rst_req   <= 1'b1;  // [FIX-4] 请求芯片硬件复位，确保芯片与 FSM 同步恢复
              fsm_state     <= S_RESET;
            end
          end

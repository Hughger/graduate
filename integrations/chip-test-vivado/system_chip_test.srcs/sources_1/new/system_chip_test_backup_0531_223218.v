...
(
  ...
  localparam S_WRITE_WEIGHTS  = 8'd10;  // 写入权重数据
  localparam S_WRITE_FEATURES = 8'd11;  // 写入特征数据
  localparam S_READ_RESULTS   = 8'd12;  // 读取计算结果
  ...
  // 数据写入相关信号
  reg [15:0] data_addr = 16'h0;      // 数据地址计数器
  (* mark_debug = "true" *) reg [10:0] weight_idx = 11'd0;     // 权重字索引 0..1151
  (* mark_debug = "true" *) reg [14:0] feature_idx = 15'd0;    // 特征字索引 0..32767
  reg [7:0]  result_idx = 8'd0;      // 结果数据索引
  
  // 权重/特征表：convert.c → weight_data.mem / feature_data.mem，综合为 Block RAM
  ...
  // Block RAM 同步读：先给 raddr，下一拍 rdata 有效后再发起总线写
  ...
  always @(posedge clk_16M or negedge ck_rst) begin
    if (!ck_rst) begin
      ...
      case (fsm_state)
        ...
        //--------------------------------------------------------------
        // S_WRITE_WEIGHTS：写入权重（BRAM 两拍：锁地址 → 用 rdata 写总线）
        //--------------------------------------------------------------
        ...
        //--------------------------------------------------------------
        // S_WRITE_FEATURES：写入特征（同上）
        //--------------------------------------------------------------
        S_WRITE_FEATURES: begin
          if (chip_input_full) begin
            fsm_en <= 1'b0;
          end else if (!rom_wr_setup) begin
            feature_raddr  <= feature_idx;
            rom_wr_setup   <= 1'b1;
          end else begin
            fsm_en       <= 1'b1;
            fsm_wr_en    <= 1'b1;
            ...
            rom_wr_setup <= 1'b0;
            if (feature_idx == 15'd32767) begin
              data_addr <= 16'h0;
        ...
        S_READ_RESULTS: begin
          ...
          if (chip_output_empty) begin
            ...
            // 更新地址和索引
            data_addr <= data_addr + 16'h8;  // 地址递增8（64位数据）
            result_idx <= result_idx + 1'b1;
            
            // 检查是否读取完成
            if (result_idx == 8'd15) begin  // 16个结果数据
              data_addr <= 16'h0;
              fsm_state <= S_CLR_INTR;
            end
          end
